...
// Function prototypes
void projectParticlesOpenCL(struct RenderContext *context);
void projectParticlesOpenCL_NoWater(struct RenderContext *context);
...
void render(struct RenderContext *context) {
	if (USE_GPU == 1) {
		...
		projectParticlesOpenCL(context);
#else
		projectParticlesOpenCL_NoWater(context);
#endif
	}
}
...
void projectParticlesOpenCL(struct RenderContext *context) {
	struct OpenCLContext *ocl = context->openCLContext;
	struct PointSOA *particles = context->particles;
	struct Camera *camera = context->camera;
	struct Triangles *triangles = context->triangles;
	struct SkyBox *skyBox = context->skyBox;
	struct GPUTimings *gpuTimings = context->gpuTimings;
	struct ImageFont *font = context->font;
	struct FireSOA *fireParticles = context->fireParticles;
	struct Missiles *missiles = context->missiles;
	cl_int err;
...
void projectParticlesOpenCL_NoWater(struct RenderContext *context) {
	...
	struct FireSOA *fireParticles = context->fireParticles;
	struct Missiles *missiles = context->missiles;
	struct IRSearchAndTrack *irst = context->irst;
	cl_int err;
	float zero = 0.0f;
...
static void initCamera(struct Camera *camera) {
...
bool initializeApp(struct AppState *state) {
	...
	InitializeMissiles(&state->missiles, MAX_FIRE_SIMS, state->missileModel); // Create 8 missiles

	for (int i = 0; i < 4; i++) {
		state->missiles.missiles[i]->position[0] = (i - 1.5f) * 300.0f;
		state->missiles.missiles[i]->position[1] = 1500.0f + (i % 2) * 200.0f;
		state->missiles.missiles[i]->position[2] = 2000.0f;
		state->missiles.missiles[i]->velocity[0] = 0.0f;
		state->missiles.missiles[i]->velocity[1] = 0.0f;
		state->missiles.missiles[i]->velocity[2] = 0.0f;
		state->missiles.missiles[i]->burning = 1;
		state->missiles.missiles[i]->burningTemp = 2000.0f;
		state->missiles.active[i] = true;
	}

	// load BVH
	ReadBVH(&state->bvh, "parseObj/encoded.bvh");
	printf("BVH loaded with %d nodes and %d triangles\n", state->bvh.NodesCount, state->bvh.TrianglesCount);

	// load font
	loadFont(&state->font, "fonts/fonts.bin");
	...
	InitializeIRST(&state->camera, &state->irst, ScreenWidth, ScreenHeight);
	...
	if (WATER_SIMULATION == 1) {
		for (int i = 0; i < NUM_PARTICLES; i++) {
			state->particles->x[i] = (float)(rand() % 50 + 30);
			state->particles->y[i] = (float)(rand() % 50);
			state->particles->z[i] = (float)(rand() % 50 + 30);
			state->particles->xVelocity[i] = (float)(rand() % 10) / 100.0f;
			state->particles->yVelocity[i] = (float)(rand() % 10) / 100.0f;
			state->particles->zVelocity[i] = (float)(rand() % 10) / 100.0f;
		}

		state->particles->bBoxMin[0] = 0.0f;
		state->particles->bBoxMin[1] = 0.0f;
		state->particles->bBoxMin[2] = 0.0f;
		state->particles->bBoxMax[0] = 80.0f;
		state->particles->bBoxMax[1] = 80.0f;
		state->particles->bBoxMax[2] = 80.0f;

		updateGridData(state->particles);
	}

	state->frameCount = 0;
	state->paused = false;
	state->fireMissile = false;
	...
	loadCurrentMap(&state->terrain, &state->camera, state->triangles);
	calculateBoundingBoxesMap(&state->terrain);
	...
	writeFileTriangles("parseObj/triangles.bin", state->triangles);
	...
	state->window = glfwCreateWindow(ScreenWidth, ScreenHeight, "OpenCL-GL Interop", NULL, NULL);
	if (!state->window) {
	...
	int useOpenCL = initializeOpenCLWithGL(&state->ocl, state->triangles, &state->skyBox, &state->font, &state->bvh, state->window, &state->missiles, state->mapGPU);
	if (!useOpenCL) {
	...
	state->sceneTriangles = (struct Triangles *)malloc(sizeof(struct Triangles));
	state->sceneTriangles->count = 0;
	dumpMap(&state->terrain, state->sceneTriangles, LOD_LOW);

	state->scene = (struct Scene *)malloc(sizeof(struct Scene));
	memset(state->scene, 0, sizeof(struct Scene));

	initGpuBuffers(state->scene, state->ocl.context, state->ocl.queue, ScreenWidth, ScreenHeight);
	loadSkyBoxForScene(state->scene);
	uploadSkyboxToGpu(state->scene);
	initGeometry(state->scene, state->sceneTriangles);
	uploadToGpu(state->scene);
...
bool appShouldExit(const struct AppState *state) {
	return state->exit || glfwWindowShouldClose(state->window);
}

void runAppFrame(struct AppState *state) {
	struct Camera *camera = &state->camera;
	struct OpenCLContext *ocl = &state->ocl;
	struct Triangles *triangles = state->triangles;
	struct GPUTimings *gpuTimings = &state->gpuTimings;
	struct TimePartition *timePartition = state->timePartition;
	struct PointSOA *particles = state->particles;
	struct FireSOA *fireParticles = state->fireParticles;
	struct Missiles *missiles = &state->missiles;
	struct IRSearchAndTrack *irst = &state->irst;
	...
	if (isMouseButtonHeld(GLFW_MOUSE_BUTTON_LEFT)) {
		// Update angles
		state->yaw += mouseState.deltaX * MouseSensitivity;
		state->pitch -= mouseState.deltaY * MouseSensitivity;
		...
		float pitchRad = state->pitch * M_PI / 180.0f;
		float yawRad = state->yaw * M_PI / 180.0f;
	...
	float dt = (float)(currentTime - state->lastTime) / (float)CLOCKS_PER_SEC; // Scale to a reasonable frame time
	float TPS = 1.0f / dt;
	...
	if (!state->paused) {
		float tmp = 0.0f;
		missilesSimulation(ocl, missiles, &tmp, &state->fireMissile, camera, 1 / 60.0f, triangles, irst);
		if (WATER_SIMULATION == 1) {
...
// === OVERLAY UI LAYER ===
	...
	if (state->frameCount < FrameCount) {
		state->averageFPS[state->frameCount] = currentFPS;
	}
	...
	if (state->frameCount >= FrameCount) {
		...
		if (fpsFile) {
			fwrite(state->averageFPS, sizeof(float), FrameCount, fpsFile);
			fclose(fpsFile);
		}
