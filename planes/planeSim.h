...
#define MAX_PLANE_MISSILES 8
#define MAX_PLANES 16
#define PLANE_CONTROL_SURFACES 6  // Aileron L/R, Elevator L/R, Rudder, Flaps
...
void InitializePlane(struct Plane *plane);
void InitializePlanes(struct Planes *planes, int count, struct Triangles *planeModel);
void CleanupPlane(struct Plane *plane);
void CleanupPlanes(struct Planes *planes);
void planeSimStep(struct Plane *plane, float deltaTime, float *timeTook);
void updatePlaneControl(struct Plane *plane, float aileronInput, float elevatorInput, float rudderInput, float throttleInput);
void planeLaunchMissile(struct Plane *plane, int missileIndex, struct Missiles *allMissiles);
void planeDeployCountermeasure(struct Plane *plane, bool deployFlare, bool deployChaff);
void planeAutopilotStep(struct Plane *plane, float deltaTime);
void planePointToDirection(struct Plane *plane, float targetDirection[3], float deltaTime, float aggressiveness);
