...
__kernel void applyReflections(
	...
	float3 environmentReflection = (screenReflectionStrength > 0.01f) ? screenSpaceReflection : skyboxReflection;
...
__kernel void renderFire(
	...
	float3 colorVariation = (float3)(random1 * 0.3f,
									 random2 * 0.2f - 0.1f,
									 random3 * 0.15f - 0.1f);
...
void rayTriangleIntersection(
	...
	float a = dot(edge1, h);
	if (a > -EPSILON && a < EPSILON) return; // Ray is parallel to triangle

	float f = 1.0f / a;
	float3 s = rayOrigin - v1;
	float u = f * dot(s, h);
	...
	if (t > EPSILON) { // Intersection
		*out_t = t;
		*out_hitNormal = normalize(cross(edge1, edge2));
	}
}
...
__kernel void renderMissile(
	...
	if (length(rotation_axis) > 0.001f) {
		...
		float3 world_p1 = (float3)(dot(row0, model_p1),
								   dot(row1, model_p1),
								   dot(row2, model_p1)) *
							  missile_scale +
						  missile_position;
		float3 world_p2 = (float3)(dot(row0, model_p2),
								   dot(row1, model_p2),
								   dot(row2, model_p2)) *
							  missile_scale +
						  missile_position;
		float3 world_p3 = (float3)(dot(row0, model_p3),
								   dot(row1, model_p3),
								   dot(row2, model_p3)) *
							  missile_scale +
						  missile_position;
		...
		float3 world_normal = normalize((float3)(dot(row0, model_normal),
...
__kernel void renderTerrainDepthLOD(
	...
	for (int i = 0; i < CHUNK_COUNT; i++) {
		...
		int highCount = (i < CHUNK_COUNT - 1) ? (mapData->chunkStartHigh[i + 1] - highStart) / 9 : (HIGH_RES_TRIANGLE_COUNT * 9 - highStart) / 9;
		int medCount = (i < CHUNK_COUNT - 1) ? (mapData->chunkStartMed[i + 1] - medStart) / 9 : (MID_RES_TRIANGLE_COUNT * 9 - medStart) / 9;
		int lowCount = (i < CHUNK_COUNT - 1) ? (mapData->chunkStartLow[i + 1] - lowStart) / 9 : (LOW_RES_TRIANGLE_COUNT * 9 - lowStart) / 9;
